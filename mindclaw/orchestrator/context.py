...
class ContextBuilder:
    ...
    def build_cron_system_prompt(
        ...
        cron_section = (
            ...
            "You are running as an unattended scheduled task (cron job). "
            ...
            "[links](url).\n"
            "- Default output limit: 3500 characters "
            "(Telegram 4096 limit after HTML conversion). "
            "Task-specific instructions may override this.\n"
            "- Structure with clear section headers. "
            "Lead with the most important info.\n"
            "- End with a forward-looking section "
            "(下期关注 / 行动建议 / 明日预告).\n\n"
            "### Data Collection\n"
            "- `web_search`: use specific keywords, mix English + Chinese "
            "queries for broader coverage.\n"
            "- `web_fetch`: for known URLs and official sources.\n"
            "- `read_file`: local files and previous reports.\n"
            "- `memory_search`: user context and preferences.\n"
            "- Target 3-5 high-quality sources per topic. "
            "Prioritize recency (today first).\n\n"
            "### Quality Bar\n"
            "- Each item: **[source]** key finding — why it matters.\n"
            "- No filler. Concrete data (numbers, dates, names) "
            "over vague descriptions.\n"
            "- Deduplicate: same story from multiple sources → one entry.\n"
