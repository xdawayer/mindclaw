...
function isStoredIdentityMaterialValid(publicKeyPem: string, privateKeyPem: string): boolean {
  try {
    const publicKey = crypto.createPublicKey(publicKeyPem);
    const privateKey = crypto.createPrivateKey(privateKeyPem);
    const probePayload = Buffer.from("openclaw-device-identity-check", "utf8");
    const signature = crypto.sign(null, probePayload, privateKey);
    return crypto.verify(null, probePayload, publicKey, signature);
  } catch {
    return false;
  }
}
  ...
  try {
    if (fs.existsSync(filePath)) {
      ...
      if (
        ...
        if (!isStoredIdentityMaterialValid(parsed.publicKeyPem, parsed.privateKeyPem)) {
          throw new Error("stored device identity key material is invalid");
        }
