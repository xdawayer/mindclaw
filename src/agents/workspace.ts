...
const SHARED_CONVERSATION_BOOTSTRAP_DENYLIST = new Set([
  DEFAULT_MEMORY_FILENAME,
  DEFAULT_MEMORY_ALT_FILENAME,
]);

function isSharedConversationSessionKey(sessionKey: string | undefined): boolean {
  const parsed = parseRawSessionConversationRef(sessionKey);
  return parsed?.kind === "group" || parsed?.kind === "channel";
}
...
export function filterBootstrapFilesForSession(
  ...
  if (sessionKey && (isSubagentSessionKey(sessionKey) || isCronSessionKey(sessionKey))) {
    filtered = files.filter((file) => MINIMAL_BOOTSTRAP_ALLOWLIST.has(file.name));
  }
  if (!sessionKey || !isSharedConversationSessionKey(sessionKey)) {
    return filtered;
  }
  // Shared conversations should not auto-inject durable private memory. Access
  // MEMORY.md on demand through memory tools instead.
  return filtered.filter((file) => !SHARED_CONVERSATION_BOOTSTRAP_DENYLIST.has(file.name));
}
