...
describe("filterBootstrapFilesForSession", () => {
  ...
  it("drops memory bootstrap files for shared group sessions", () => {
    const result = filterBootstrapFilesForSession(mockFiles, "agent:main:feishu:group:oc_demo");
  ...
  it("drops memory files for channel-type shared sessions", () => {
    const result = filterBootstrapFilesForSession(mockFiles, "agent:main:slack:channel:C12345");
    const names = result.map((file) => file.name);
    expect(names).toContain("AGENTS.md");
    expect(names).not.toContain("MEMORY.md");
    expect(names).not.toContain("memory.md");
  ...
  it("keeps memory files for direct/private sessions", () => {
    const result = filterBootstrapFilesForSession(mockFiles, "agent:main:feishu:direct:ou_user_1");
    const names = result.map((file) => file.name);
